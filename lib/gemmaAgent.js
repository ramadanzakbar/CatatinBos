...
function getFreshGcloudToken() {
  try {
    const token = execSync('gcloud auth print-access-token', { timeout: 2500, encoding: 'utf-8' }).trim();
    if (token && token.startsWith('ya29.')) {
      return token;
    }
  } catch (e) {
    // Ignore if gcloud is not installed or not authenticated
  }
  return null;
}
...
export async function callGemmaAgent({ prompt, imageBase64, tools }) {
  ...
  // Always attempt to get a fresh gcloud CLI token if local bearerToken is missing or ya29 token
  const freshGcloudToken = getFreshGcloudToken();
  if (freshGcloudToken) {
    bearerToken = freshGcloudToken;
  }

  // Auto-fetch GCP Metadata Server service account token if running inside Cloud Run / GCP
...
KEPRIBADIAN & GAYA BAHASA (RAMAH & HIDUP):
- Berbicaralah dengan gaya bahasa yang hangat, ramah, manusiawi, bersemangat, solutif, dan perhatian seperti seorang Financial Advisor pribadi yang terpercaya.
- SETELAH MEMANGGIL TOOL CALLING / MELAKUKAN AKSI (seperti mencatat transaksi, set budget, split bill, proyeksi), SELALU BERIKAN PENJELASAN RAMAH TENTANG APA YANG SUDAH KAMU LAKUKAN, beserta saran finansial atau pertanyaan lanjutan dengan emoji yang relevan (📝, 📊, 🚀, 💡, 📲).
...
5. JANGAN PERNAH meminta pengguna memasukkan data manual jika pengguna meminta proyeksi, analisis, atau ringkasan yang bisa dihitung dari database!`;
  ...
  // Helper intent trigger & NLU fallback for 100% agent resilience
  const resolveIntentTools = (promptText, existingCalls) => {
    if (existingCalls && existingCalls.length > 0) return existingCalls;
    
    // Extract raw user prompt if memory prompt was passed
    let cleanPrompt = promptText;
    if (cleanPrompt.includes('Pesan Pengguna Saat Ini:')) {
      cleanPrompt = cleanPrompt.split('Pesan Pengguna Saat Ini:').pop().trim();
    }
    const pLower = cleanPrompt.toLowerCase();

    // 1. Transaction Intent (e.g. "Catat pengeluaran makan siang Rp50.000")
    if (pLower.includes('catat') || pLower.includes('pengeluaran') || pLower.includes('pemasukan') || pLower.includes('bayar') || pLower.includes('beli') || pLower.includes('gaji')) {
      let type = pLower.includes('pemasukan') || pLower.includes('gaji') || pLower.includes('terima') || pLower.includes('dapat') ? 'INCOME' : 'EXPENSE';
      
      // Amount extraction (e.g. Rp50.000 or 50rb or 50000)
      let amount = 0;
      const numMatch = cleanPrompt.match(/(?:rp\.?\s*)?(\d+(?:[\.,]\d+)*)(?:\s*(rb|k|ribu|juta|jt))?/i);
      if (numMatch) {
        let rawNum = numMatch[1].replace(/\./g, '').replace(',', '.');
        let val = parseFloat(rawNum) || 0;
        const multiplier = numMatch[2] ? numMatch[2].toLowerCase() : '';
        if (multiplier === 'rb' || multiplier === 'k' || multiplier === 'ribu') val *= 1000;
      ...
      if (pLower.includes('makan') || pLower.includes('kopi') || pLower.includes('resto') || pLower.includes('siang') || pLower.includes('malam') || pLower.includes('snack')) category = 'Food';
      else if (pLower.includes('bensin') || pLower.includes('grab') || pLower.includes('gojek') || pLower.includes('trans') || pLower.includes('tol') || pLower.includes('parkir')) category = 'Transport';
      else if (pLower.includes('gaji') || pLower.includes('freelance') || pLower.includes('bonus')) category = 'Salary';
      else if (pLower.includes('sewa') || pLower.includes('listrik') || pLower.includes('air') || pLower.includes('wifi') || pLower.includes('pulsa')) category = 'Bills';
      else if (pLower.includes('baju') || pLower.includes('sepatu') || pLower.includes('belanja')) category = 'Shopping';
      else if (pLower.includes('nonton') || pLower.includes('film') || pLower.includes('game') || pLower.includes('netflix')) category = 'Entertainment';

      if (amount > 0) {
        return [{ name: 'add_transaction', args: { amount, category, type, note: cleanPrompt } }];
      }
    }

    // 2. Budget Limit Intent
    if (pLower.includes('budget') || pLower.includes('pagu') || pLower.includes('batas')) {
      let category = 'Food';
      if (pLower.includes('transport')) category = 'Transport';
      if (pLower.includes('shopping') || pLower.includes('belanja')) category = 'Shopping';
      if (pLower.includes('entertainment') || pLower.includes('hiburan')) category = 'Entertainment';
      ...
      return [{ name: 'set_budget_limit', args: { category, limitAmount: amount } }];
    }

    // 3. Proyeksi & Analysis Intent
